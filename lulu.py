...
# Set up the web page
st.set_page_config(page_title="Universal PO to KR Code Mapper", layout="wide")
...
def process_pdf(pdf_file):
    ...
    try:
        ...
        # SMART DYNAMIC HEADER DETECTION FOR PO (LuLu & Nesto)
        header_row_idx = None
        for idx, row in df.iterrows():
            row_str = " ".join([str(val).upper() for val in row.values if pd.notnull(val)])
            # Check for either LuLu or Nesto column signatures
            if ("QUANTITY" in row_str or "ORD.QTY" in row_str) and ("UOM" in row_str or "UNIT" in row_str):
                header_row_idx = idx
                break
                
        if header_row_idx is None:
            st.warning("Could not automatically find the Quantity and UOM headers. Returning raw data.")
            return df 
        ...
        # DYNAMIC TRUE QUANTITY CALCULATION (Adapts to UOM or UnitConv)
        def calculate_true_quantity(row):
            try:
                # Dynamically fetch Quantity
                ...
                uom_val = row.get('UOM') if 'UOM' in row.index else (row.get('UnitConv.') if 'UnitConv.' in row.index else row.get('Unit', ''))
...
def apply_mappings(df_po, master_file, order_form_file):
    try:
        # ==========================================
        # 1. LOAD MASTER FILE (ZDET-PRICE) - Universal
        # ==========================================
        ...
        # ==========================================
        # 2. LOAD ORDER FORM (FALLBACK) - Dynamic for LuLu & Nesto
        # ==========================================
        ...
        if order_form_file is not None:
            ...
            for sheet_name, df_sheet in df_order_dict.items():
                for idx, row in df_sheet.iterrows():
                    ...
                    # Look for LuLu OR Nesto barcode/KR Code columns
            ...
            if df_order is not None:
                ...
                df_order.columns = [str(c).strip().upper() for c in df_order.columns] # Normalize to uppercase
                ...
                st.success(f"✅ Order Form Fallback loaded! (Found: {of_retailer} & {of_kr})")
            else:
                st.warning("⚠️ Could not find Barcode & KR Code headers in the Order Form. Skipping fallback.")
        ...
        def find_kr_code(row):
            ...
            # Layer 3: Order Form Retailer Code (LuLu Code / Nesto Code matched to PO Article)
        ...
        # Output the correct columns based on the file type
        desired_columns = ['KR CODE', 'True Quantity', 'Barcode', 'Article']
        if po_desc_col: desired_columns.append(po_desc_col) # Adds either LuLu or Nesto description name
...
# Run the pipeline only when Master and PO are provided
